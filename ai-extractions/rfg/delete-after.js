// ------------------ PROMPT GENERATOR ------------------
// Generates the Exclusions & Limitations prompt with the correct field reference
function getExclusionLimitationPrompt(qaFieldValue) {
  return `<GROUND_RULES>
...
// ------------------ SET PROMPTS FOR ALL VARIANTS ------------------
const VARIANT_SUFFIXES = ['', '_Renewal', '_AP1', '_AP2', '_AP3', '_AP4'];

VARIANT_SUFFIXES.forEach((suffix) => {
  const qaField = feathery.fields[`AP_LTDExclusionLimitationQA${suffix}`];
  const promptField = feathery.fields[`AP_LTDprompt5${suffix}`];
  
  if (!qaField || !promptField) {
    console.log(`[SetPrompt] Skipping suffix "${suffix || 'main'}" - field not found`);
    return;
  }
  
  promptField.value = getExclusionLimitationPrompt(qaField.value || '');
  console.log(`[SetPrompt] Set AP_LTDprompt5${suffix}`);
});

console.log('[SetPrompt] All prompts set');
